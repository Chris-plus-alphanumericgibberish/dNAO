...
{
	return (boolean)((is_u(etmp) ? Passes_walls :
			passes_walls_mon(etmp->emon)) || noncorporeal(etmp->edata));
}
	...
	if (is_db_wall(etmp->ex, etmp->ey)) {
		...
		if (is_pool(etmp->ex, etmp->ey, TRUE) && !e_inview)
			if (flags.soundok)
				You_hear("a splash.");
		if (e_survives_at(etmp, etmp->ex, etmp->ey)) {
			if (e_inview && !is_flyer_mon(etmp->emon) &&
			    !is_floater_mon(etmp->emon))
