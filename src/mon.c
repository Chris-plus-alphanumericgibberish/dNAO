...
register struct monst *mtmp;
{
	...
	inpool = is_pool(mtmp->mx,mtmp->my, FALSE) &&
	     ((!is_flyer_mon(mtmp) && !is_floater_mon(mtmp)) || is_3dwater(mtmp->mx,mtmp->my));
	inlava = is_lava(mtmp->mx,mtmp->my) &&
	     !is_flyer_mon(mtmp) && !is_floater_mon(mtmp);
	...
	if (!is_clinger(mtmp->data)
	    && !is_swimmer_mon(mtmp) && !amphibious_mon(mtmp)) {
	    if (cansee(mtmp->mx,mtmp->my)) {
...
#ifdef INVISIBLE_OBJECTS
		if (otmp->oinvis && !perceives_mon(mtmp)) continue;
#endif
		if (cansee(mtmp->mx,mtmp->my) && flags.verbose)
	...
	struct monst *lookie;
{
	...
	if((darksight(looker->data) || (catsight(looker->data) && catsightdark)) && !is_blind(looker)){
		...
		if(clear_path(looker->mx, looker->my, lookie->mx, lookie->my) && !(lookie->minvis && !perceives_mon(looker) && !can_track(looker->data) && rn2(11))){
	...
	if(infravision(looker->data) && infravisible(youracedata) && !is_blind(looker)){
		if((clear_path(looker->mx, looker->my, lookie->mx, lookie->my) || ominsense(looker->data)) && !(lookie->minvis && !perceives_mon(looker) && !can_track(looker->data) && rn2(11))){
			return TRUE;
		}
	}
	...
	if(earthsense(looker->data) && !(is_flyer_mon(lookie) || is_floater_mon(lookie) || unsolid(lookie->data))){
		return TRUE;
	}
	if(telepathic_mon(looker)){
		return TRUE;
	}
	...
	struct monst *looker;
{
	...
	if((darksight(looker->data) || (catsight(looker->data) && catsightdark)) && !is_blind(looker)){
		if(couldsee(looker->mx, looker->my) && !(Invis && !perceives_mon(looker) && !can_track(looker->data) && rn2(11))){
	...
	if(infravision(looker->data) && infravisible(youracedata) && !is_blind(looker)){
		if((couldsee(looker->mx, looker->my) || ominsense(looker->data)) && !(Invis && !perceives_mon(looker) && !can_track(looker->data) && rn2(11))){
			return TRUE;
		}
	}
	if(bloodsense(looker->data) && has_blood(youracedata)){
	...
	long flag;
{
	...
	cubewaterok = (is_swimmer_mon(mon) || breathless_mon(mon) || amphibious_mon(mon));
	poolok = is_flyer_mon(mon) || is_clinger(mdat) ||
		 (is_swimmer_mon(mon) && !wantpool);
	lavaok = is_flyer_mon(mon) || is_clinger(mdat) || likes_lava(mdat);
...
nexttry:	/* eels prefer the water, but if there is no water nearby,
	...
	for(nx = max(1,x-1); nx <= maxx; nx++)
	  for(ny = max(0,y-1); ny <= maxy; ny++) {
		...
		int dispx, dispy;
		boolean monseeu = (!is_blind(mon) && (!Invis || perceives_mon(mon)));
