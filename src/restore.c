...
//			mread(fd, (genericptr_t) otmp->mp->mskamask, (unsigned) sizeof(struct attribs));
		...
		if(u.uz.dlevel && !u.silverknight_mire && otmp->mired){
			otmp->oeroded = 0;
			otmp->oeroded2 = 0;
...
restgamestate(int fd, unsigned int *stuckid, unsigned int *steedid, unsigned int *riderid)
{
	...
	/* Fix up the highest ranking eladrins; must come after restore_dungeon
	   has loaded the real dungeon_topology.alt_tulani from the save file */
