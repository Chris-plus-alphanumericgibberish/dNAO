...
struct monst *mon;
{
	...
	if(couldsee(mon->mx, mon->my) && !is_blind(mon)){
		...
		if(u.sealsActive&SEAL_ANDROMALIUS && !NoBInvis && (!(levl[u.ux][u.uy].lit == 0 && !(viz_array[u.uy][u.ux]&TEMP_LIT1 && !(viz_array[u.uy][u.ux]&TEMP_DRK3))) || (ublindf->otyp==MASK || ublindf->otyp==R_LYEHIAN_FACEPLATE))) count++;
		// if(u.sealsActive&SEAL_ASTAROTH && !Invis && !(ublindf && ublindf->otyp != LENSES)) count++;
		...
		// if(u.sealsActive&SEAL_ECHIDNA && !Invis && !(uarmf && (uarmc || uarm))) count++;
		if(u.sealsActive&SEAL_EDEN && !NoBInvis && !(!verysmall(youracedata) || !uarmh) && !bigmonst(youracedata) && !uarmh) count++;
		// if(u.sealsActive&SEAL_ENKI && !Invis && !(uarm || uarmc)) count++;
