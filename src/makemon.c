	    ...
	    case S_HUMAN:
			if(mm == PM_ELDER_PRIEST){
				...
				if(mm != PM_DROW_MATRON && mm != PM_DROW_MATRON_MOTHER && mm != PM_DROW_NOVICE 
					...
					(void)mongets(mtmp, DROVEN_DAGGER);
					(void)mongets(mtmp, DROVEN_DAGGER);
					otmp = mksobj(DROVEN_SHORT_SWORD, TRUE, FALSE);
					otmp->spe = 7;
...
// endif
					...
					(void) mongets(mtmp, !rn2(10) ? SILVER_KHAKKHARA : MACE);
				...
				if(mm == PM_LORD_CARNARVON){
					...
					otmp = mksobj(LARGE_SHIELD, FALSE, FALSE);
					bless(otmp);
					otmp->spe = 0;
					(void) mpickobj(mtmp, otmp);
					...
					otmp = mksobj(LEATHER_GLOVES, FALSE, FALSE);
					bless(otmp);
					otmp->spe = 0;
					(void) mpickobj(mtmp, otmp);
					...
					otmp = mksobj(SILVER_KHAKKHARA, FALSE, FALSE);
					bless(otmp);
					otmp->spe = 7;
					(void) mpickobj(mtmp, otmp);
