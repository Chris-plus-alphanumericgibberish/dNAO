...
SRCOBJ = allmain.o alloc.o apply.o artifact.o astar.o attkbp.o attrib.o ball.o bones.o	\
         ...
         mattkbp.o mextra.o minion.o mklev.o mkmap.o mkmaze.o mkobj.o mkroom.o	\
...
MAKEDEFS_O = util/makedefs.o util/atkbpnames.o src/alloc.o src/monst.o	\
             src/objects.o util/panic.o
util/makedefs: $(MAKEDEFS_O)
	$(CC) $(LDFLAGS) $^ $(LDLIBS) -o $@
AUTO_BIN += util/makedefs

# built and run strictly after include/pm.h and include/atkbpbits.h exist
...
ALL_O = $(GAME_O) $(RECOVER_O) $(MAKEDEFS_O) $(ATTKBPDEFS_O) $(DLB_O) $(DGN_COMP_O)	\
...
include/atkbpbits.h: util/makedefs
	cd util && ./makedefs -k  # include/atkbpbits.h
...
AUTO_H += include/date.h include/onames.h include/gnames.h include/pm.h include/atkbpbits.h include/verinfo.h include/macromagic.h
...
src/attkbp.c: util/attkbpdefs
	cd util && ./attkbpdefs  # src/attkbp.c
AUTO_C += src/attkbp.c
